...
void process_free(struct process *process, void *ptr)
{
    ...
    if (!allocation) {
        // The pointer is not allocated by the process
        return;
    }
...
static int process_load_binary(const char *filename, struct process *process)
{
    ...
    process->file_type = PROCESS_FILETYPE_BINARY;
...
static int process_load_elf(const char *filname, struct process *process)
{
    int res = 0;
    struct elf_file *elf_file = 0;
    res = elf_load(filname, &elf_file);
    if (ISERR(res)) {
        goto out;
    }

    process->file_type = PROCESS_FILETYPE_ELF;
...
int process_map_memory(struct process *process)
{
    ...
    switch (process->file_type) {
        ...
        case PROCESS_FILETYPE_BINARY:
            ...
            break;
        default:
            panic("process_map_memory: Unknown file type\n");
    }
