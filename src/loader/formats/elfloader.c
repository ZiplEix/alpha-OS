...
{
    // Only support little endian elf files
...
{
    return header->e_type == ET_EXEC && header->e_entry >= ALPHAOS_PROGRAM_VIRTUAL_ADDRESS;
}
...
int elf_process_phdr_pt_load(struct elf_file *elf_file, struct elf32_phdr *phdr)
{
    ...
    unsigned int end_virtual_address = phdr->p_paddr + phdr->p_filesz;
